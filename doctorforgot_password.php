...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_query($con, "SELECT doctor_id FROM doctors WHERE username='$username' AND email='$email'");
    $row = mysqli_num_rows($query);
    
    if ($row > 0) {
        $_SESSION['username'] = $username;
        $_SESSION['email'] = $email;
        header('Location: doctorreset.php');
    } else {
        echo "<script>alert('Invalid details. Please try with valid details');</script>";
        echo "<script>window.location.href ='doctorforgot_password.php';</script>";
    }
}
?>
...
<body>
    <div class="container">
        <div class="logo">
            <h2>MedAlert Access System | Doctor Password Recovery</h2>
        </div>
        <form method="post">
            <fieldset>
